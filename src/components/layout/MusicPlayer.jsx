...
export default function MusicPlayer({ musicQueue }) {
  ...
  // Get current song from musicQueue (for non-podcast items, use album songs)
  let currentSong;
  ...
  // Determine the length (number of songs)
  ...
  // Animate the progress bar when playing
  useEffect(() => {
    let interval;
    if (isPlaying) {
  ...
  // Restart playback when musicQueue changes
  useEffect(() => {
    if (musicQueue?.index !== undefined) {
      setCurrentSongIndex(musicQueue.index);
      setRangeValue(0);
    }
    // Toggle play state (if needed, adjust logic as required)
    setIsPlaying(prev => !prev);
  }, [musicQueue]);

  // Calculate total duration in seconds (fallback to 195 seconds if undefined)
  ...
  // Handle clicks on the progress range input
        ...
        {musicQueue != null && (
            <div className="fixed bottom-0 bg-black w-full h-[10vh] md:h-[5vw] overflow-x-hidden z-10 px-4 py-2 flex items-center justify-between transition-all">
              ...
              <div className="flex items-center space-x-4 h-full">
                <img
                    // Use current song's image if available; otherwise fallback to album/podcast image
                    src={
                        currentSong?.image ||
